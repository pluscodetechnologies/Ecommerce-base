  ...
  try {
    ...
    const user = JSON.parse(
      JSON.stringify(
        (
          await db.execute(
            "SELECT name, email, totp_enabled FROM users WHERE id = ?",
            [req.userId],
          )
        )[0][0],
      ),
    ...
    if (user.totp_enabled) {
      return res
        .status(400)
        .json({ success: false, message: "2FA já está ativo." });
    }
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (!code)
      return res
        .status(400)
        .json({ success: false, message: "Código obrigatório" });
    ...
    if (!rows.length || !rows[0].totp_secret) {
      return res
        .status(400)
        .json({ success: false, message: "Execute o setup primeiro" });
    }
...
router.post("/disable", async (req, res) => {
  try {
    ...
    if (!code)
      return res
        .status(400)
        .json({ success: false, message: "Confirme com o código atual" });
    ...
    if (!rows.length || !rows[0].totp_enabled) {
      return res
        .status(400)
        .json({ success: false, message: "2FA não está ativo" });
    }
