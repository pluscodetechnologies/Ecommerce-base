...
async function connectDB() {
  try {
    ...
    try {
      ...
      if (e.errno !== 1060)
        console.warn("Migration sort_order products:", e.message);
    }
    ...
    try {
      ...
      if (e.errno !== 1060)
        console.warn("Migration sort_order store_alerts:", e.message);
    }
    ...
    try {
      ...
      if (e.errno !== 1060)
        console.warn("Migration sort_order product_colors:", e.message);
    }
...
function getDB() {
  if (!pool) {
    throw new Error(
      "Banco de dados não inicializado. Chame connectDB() primeiro.",
    );
  }
  return pool;
}
