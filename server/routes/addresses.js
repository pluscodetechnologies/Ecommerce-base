...
// POST /api/addresses — salvar endereço manualmente
router.post('/', async (req, res) => {
    try {
        ...
        const db = getDB();
        if (is_default) {
            await db.execute('UPDATE user_addresses SET is_default = 0 WHERE user_id = ?', [req.userId]);
        }
        const [result] = await db.execute(
            `INSERT INTO user_addresses (user_id, street, number, complement, neighborhood, city, state, zip_code, is_default)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [req.userId, street, number, complement || null, neighborhood, city, state, zip_code, is_default ? 1 : 0]
        );
        res.json({ success: true, data: { id: result.insertId } });
    } catch (e) {
        console.error('[addresses.post]', e);
        res.status(500).json({ success: false, message: 'Erro ao salvar endereço' });
    }
});

// PUT /api/addresses/:id/default — marcar como padrão
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Erro ao remover endereço' });
    }
});
