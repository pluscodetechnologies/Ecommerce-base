...
window.Wishlist = {
    ...
    // Helper: usa auth.fetchWithAuth se disponível, senão fetch direto
    ...
    async getIds() {
        ...
        try {
            ...
            if (res.status === 401) return new Set(); // não logado
    ...
    async toggle(productId) {
        ...
        try {
            const res  = await this._fetch(`/api/wishlist/${productId}`, { method: 'POST' });
            const data = await res.json();
