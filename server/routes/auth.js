...
// Verificação TOTP (passo 2 do login admin com 2FA)
router.post('/2fa-login',
    ...
    async (req, res) => {
        try {
            ...
            if (!valid) {
                return res.status(401).json({ success: false, message: 'Código inválido. Tente novamente.' });
            }

            // Limpa o token temporário
            await db.execute(
            ...
            const { token: refreshToken, expiresAt } =
                await refreshTokenService.createRefreshToken(user.id, { ip, userAgent, rememberMe: rememberMe === true });
