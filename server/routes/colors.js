...
router.get('/:productId', async (req, res) => {
    try {
        ...
        // Tenta com sort_order; se a coluna não existir, cai no fallback
        try {
            ...
            // Coluna sort_order ainda não existe — ordena só por name
            [colors] = await db.execute(
...
router.post('/:productId', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    console.log('[colors POST] productId:', productId, '| cores recebidas:', JSON.stringify((colors||[]).map((c,i) => ({ i, name: c.name, sort_order: c.sort_order }))));
    ...
    try {
        ...
        if (colors && colors.length) {
            ...
            // Verifica se sort_order existe na tabela
            let hasSortOrder = true;
            try {
                await connection.execute('SELECT sort_order FROM product_colors LIMIT 0');
            } catch (e) {
                hasSortOrder = false;
            }
                ...
                if (hasSortOrder) {
                    await connection.execute(
                        `INSERT INTO product_colors (product_id, name, hex, stock, images, sort_order)
                         VALUES (?, ?, ?, ?, ?, ?)`,
                        [productId, c.name.trim(), c.hex || null, parseInt(c.stock) || 0, JSON.stringify(imgs), sortOrder]
                    );
                } else {
                    await connection.execute(
                        `INSERT INTO product_colors (product_id, name, hex, stock, images)
                         VALUES (?, ?, ?, ?, ?)`,
                        [productId, c.name.trim(), c.hex || null, parseInt(c.stock) || 0, JSON.stringify(imgs)]
                    );
                }
            }
        }
        ...
        res.status(500).json({ success: false, message: 'Erro ao salvar cores: ' + e.message });
    } finally {
        connection.release();
    }
});
